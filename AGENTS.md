...
- Lead forms must POST JSON to the existing VPS endpoint `/api/brief` using the required `name`, `contact`, and `message` fields plus the `website` honeypot. Never show a success state before the API confirms `{ ok: true }`; preserve retryable error feedback and direct-contact fallbacks.
