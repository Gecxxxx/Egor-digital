...
function Hero({ go, onLead }) {
  const headline = ["Сайты для", "бизнеса от $500,", "которые объясняют", "услугу и приводят", "заявки"];
  return <><section className="hero grid-surface"><div className="hero-copy"><Eyebrow>Digital Tools by Egor</Eyebrow><h1 className="hero-business-title" aria-label="Сайты для бизнеса от $500, которые объясняют услугу и приводят заявки">{headline.map((line, index) => <span className={index === headline.length - 1 ? "hero-line accent" : "hero-line"} key={line}><span>{line}</span></span>)}</h1><p className="hero-sub">Продумываю структуру, дизайн, мобильную версию и формы. <strong>CRM и автоматизацию можно добавить сразу или после запуска.</strong></p><div className="hero-actions"><CTA href="/pricing" go={go}>Рассчитать стоимость</CTA><CTA secondary onClick={() => onLead({ service: "audit", serviceLabel: "Аудит", source: "hero_audit" })}>Получить разбор сайта</CTA></div><div className="hero-facts"><div><b>Реальные проекты</b><strong><CountUp end={3} suffix=" кейса" /></strong></div><div><b>География</b><strong>Россия и зарубежье</strong></div><div><b>Канал заявок</b><strong>Telegram и CRM</strong></div><div><b>После запуска</b><strong><CountUp end={2} suffix=" месяца поддержки" /></strong></div></div><p className="hero-clients">Green Apple Dent · Дарья Каминскене · Крыша-мечты</p></div><div className="hero-person"><img src="/assets/images/egor-hero-cutout.webp" alt="Егор — разработчик сайтов и CRM" width="1086" height="1448" loading="eager" decoding="sync" fetchPriority="high" /></div></section><div className="ticker" aria-label="Преимущества"><div className="ticker-track">{[...tickerItems, ...tickerItems].map((item, index) => <span aria-hidden={index >= tickerItems.length ? "true" : undefined} key={`${item}-${index}`}>{item}</span>)}</div></div></>;
}
