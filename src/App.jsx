...
const serviceRows = [
  ["01", "Полный аудит и доработка", "Разбираю структуру, доверие, мобильную версию, формы и аналитику, затем исправляю приоритетные проблемы.", "от 25 000 ₽"],
  ["02", "Сайт для бизнеса", "Лендинг или многостраничный сайт: структура, дизайн, адаптив, формы заявок и аналитика.", "от 50 000 ₽"],
  ["03", "CRM", "Клиенты, заявки, статусы, ответственные и история общения в единой системе.", "от 100 000 ₽"],
  ["04", "Автоматизация", "Уведомления, интеграции, отчёты и автоматические сценарии для работы с заявками.", "от 120 000 ₽"],
  ["05", "Поддержка", "Исправления, новые блоки, контроль работы форм и техническое развитие после запуска.", "от 15 000 ₽/мес"],
  ["06", "Дополнительные задачи", "Отдельные страницы, формы, квизы, интеграции и другие точечные работы.", "по задаче"],
];
...
function AnimatedChars({ text, offset = 0 }) {
  let cursor = offset;
  return <span className="char-group" aria-label={text}>{text.split(/(\s+)/).map((token, tokenIndex) => {
    const tokenOffset = cursor;
    cursor += [...token].length;
    if (/^\s+$/.test(token)) return <span className="char-space" aria-hidden="true" key={`space-${tokenIndex}`}>{token}</span>;
    return <span className="char-word" aria-hidden="true" key={`${token}-${tokenIndex}`}>{[...token].map((char, index) => <span className="char" style={{ "--char-index": tokenOffset + index }} key={`${char}-${index}`}>{char}</span>)}</span>;
  })}</span>;
}
...
function CountUp({ end, prefix = "", suffix = "" }) {
  ...
  const formattedValue = new Intl.NumberFormat("ru-RU").format(value).replace(/\u00a0/g, " ");
  return <span ref={ref} className="count-up">{prefix}{formattedValue}{suffix}</span>;
}
...
function Services({ onLead }) { return <main><PageHero eyebrow="Услуги" title="Услуги для сайта, заявок и CRM" text="Начните с сайта, полного аудита или доработки, а затем добавьте CRM, аналитику и автоматизацию. Выбираем то, что решает задачу бизнеса." onLead={onLead} /><section className="section"><Eyebrow>С чего начать</Eyebrow><AccentTitle>Направления работы</AccentTitle><div className="service-table">{serviceRows.map(([n,t,d,p]) => <div className="service-row" key={n}><span>{n}</span><h3>{t}</h3><p>{d}</p><strong>{p}</strong></div>)}</div></section><section className="section dark-band"><Eyebrow>Популярные сценарии</Eyebrow><div className="offer-grid"><Offer title="Сайт для бизнеса" price="от 50 000 ₽" text="Страница или небольшой сайт, который объясняет услугу, вызывает доверие и ведёт к заявке." /><Offer title="Сайт + CRM" price="от 100 000 ₽" text="Источник, статус, следующий шаг и история общения сохраняются в системе." /><Offer title="CRM / Автоматизация" price="от 120 000 ₽" text="Клиенты, источники, статусы, уведомления и отчёты в одной системе." /><Offer title="Поддержка" price="от 15 000 ₽/мес" text="Исправления, новые блоки, связки и техническое развитие после запуска." /></div></section><FinalCta onLead={onLead} /></main>; }
...
const plans = [{ label: "Точка входа", title: "Аудит и доработка", price: "25 000 ₽", service: "audit", serviceLabel: "Аудит", text: "Полный разбор сайта и исправление приоритетных проблем.", bullets: ["структура и доверие", "мобильная версия", "формы и аналитика", "приоритетные доработки"] }, { label: "Сайт", title: "Сайт для бизнеса", price: "50 000 ₽", service: "website", serviceLabel: "Сайт", text: "Структура, дизайн, адаптив, форма и один канал заявок.", bullets: ["структура под доверие", "телефон и компьютер", "форма и аналитика", "Telegram, MAX или email"] }, { label: "Сайт + система", title: "Сайт + CRM", price: "100 000 ₽", service: "crm", serviceLabel: "CRM", text: "Сайт и единая система для приёма и контроля обращений.", bullets: ["сайт и формы", "CRM или админка", "статусы заявок", "уведомления"] }, { label: "Автоматизация", title: "CRM / Автоматизация", price: "120 000 ₽", service: "crm", serviceLabel: "CRM", text: "Клиенты, источники, статусы, интеграции и отчёты в одной системе.", bullets: ["CRM или админка", "статусы и заявки", "интеграции и уведомления", "экспорт и аналитика"] }];

function Pricing({ onLead }) { return <main><PageHero className="pricing-hero" eyebrow="Цены" title="Цены на сайт, заявки и CRM" text="Можно запустить сайт от 50 000 ₽, а CRM, автоматизацию и поддержку добавить по мере роста. До старта понятно, что входит в работу." onLead={onLead} /><section className="section pricing-plans-section"><Eyebrow>Стартовый формат</Eyebrow><AccentTitle>Выберите точку входа</AccentTitle><div className="plans">{plans.map((p) => <article className="plan" key={p.title}><p>{p.label}</p><h3>{p.title}</h3><div><strong>от {p.price}</strong></div><span>{p.text}</span><ul>{p.bullets.map((b) => <li key={b}>{b}</li>)}</ul><CTA onClick={() => { trackGoal("pricing_plan_select", getGoalContext({ plan: p.title, price: p.price, service: p.service })); onLead({ service: p.service, serviceLabel: p.serviceLabel, price: `от ${p.price}`, selectionLabel: `${p.title} — от ${p.price}`, source: "pricing" }); }}>Выбрать</CTA></article>)}</div></section><section className="section pricing-detail"><div><Eyebrow>Входит в сайт от 50 000 ₽</Eyebrow><ul><li>структура под задачу бизнеса</li><li>дизайн в едином стиле</li><li>адаптив под основные устройства</li><li>форма и один канал заявок</li><li>SEO-теги и аналитика</li></ul></div><div><Eyebrow>Сроки</Eyebrow><dl><div><dt>от 1 дня</dt><dd>небольшая задача</dd></div><div><dt>3–7 дней</dt><dd>мини-лендинг</dd></div><div><dt>от 7 дней</dt><dd>сайт для бизнеса</dd></div><div><dt>от 2 недель</dt><dd>CRM / админка</dd></div></dl></div></section><Faq /><FinalCta onLead={onLead} /></main>; }
