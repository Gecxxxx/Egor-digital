...
const serviceRows = [
  ["01", "Сайты и страницы", "Лендинги, многостраничные сайты, страницы услуг и доработки текущего сайта под заявки.", "от 50 000 ₽"],
  ["02", "Аудит и структура", "Находим, что мешает доверию и заявкам, проектируем понятную структуру до сборки.", "от 20 000 ₽"],
  ["03", "Формы и квизы", "Контактные формы, кнопки связи, мини-опросы и передача обращений в нужный канал.", "по задаче"],
  ["04", "CRM и учёт", "Клиенты, статусы, ответственные и история общения в одной системе вместо хаоса в чатах.", "от 100 000 ₽"],
  ["05", "Автоматизация", "Telegram-уведомления, интеграции, отчёты и развитие системы после запуска.", "от 15 000 ₽/мес"],
];
...
function Hero({ go, onLead }) { return <><section className="hero grid-surface"><div className="hero-copy"><Eyebrow>Digital Tools by Egor</Eyebrow><h1 aria-label="Сайт. Заявки. Контроль."><span className="hero-line"><span>Сайт.</span></span><span className="hero-line"><span>Заявки.</span></span><span className="hero-line accent"><span>Контроль.</span></span></h1><p className="hero-sub"><AnimatedChars text="Создаю сайты и подключаю CRM, " /><strong><AnimatedChars text="чтобы бизнес получал обращения и не терял клиентов" offset={34} /></strong></p><div className="hero-actions"><CTA onClick={() => onLead({ service: "website", serviceLabel: "Сайт", source: "hero_website" })}>Мне нужен сайт</CTA><CTA secondary onClick={() => onLead({ source: "hero_audit" })}>Получить разбор</CTA><CTA secondary href="/cases" go={go}>Кейсы</CTA></div><div className="hero-facts"><div><b>Сайт для бизнеса</b><strong>от <CountUp end={50000} suffix=" ₽" /></strong></div><div><b><CountUp end={2} suffix=" месяца поддержки" /></b><strong>бесплатно</strong></div></div></div><div className="hero-person"><img src="/assets/images/egor-hero-cutout.webp" alt="Егор — разработчик сайтов и CRM" width="1086" height="1448" loading="eager" decoding="sync" fetchPriority="high" /></div></section><BenefitTicker /></>; }
...
const plans = [{ label: "Старт", title: "Маленькая задача", price: "20 000 ₽", service: "audit", serviceLabel: "Аудит", text: "Аудит, правка, форма или небольшой модуль.", bullets: ["аудит сайта или формы", "доработка блока и адаптива", "форма заявки", "уведомления по задаче"] }, { label: "Лучший выбор", title: "Сайт для бизнеса", price: "50 000 ₽", service: "website", serviceLabel: "Сайт", text: "Структура, дизайн, адаптив, форма и один канал заявок.", bullets: ["структура под доверие", "телефон и компьютер", "форма и аналитика", "Telegram, MAX или email"] }, { label: "Система контроля", title: "CRM / Автоматизация", price: "100 000 ₽", service: "crm", serviceLabel: "CRM", text: "Клиенты, источники, статусы и отчёты в одной системе.", bullets: ["CRM или админка", "статусы и заявки", "уведомления", "экспорт и аналитика"] }];
